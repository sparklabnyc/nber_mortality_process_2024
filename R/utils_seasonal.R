...
f <- function(year, race_levels, race_labels) {
  ...
  output_rds_path <- file.path("output_seasonal", paste0("summary_", year, ".rds"))
  saveRDS(summary_data, file = output_rds_path)
}


f1 <- function(year, race_levels, race_labels) {
  file_path <- file.path("data", paste0("mort", year, ".csv"))
  data <- read.csv(file_path, fileEncoding="latin1")
...
f2 <- function(year, race_levels, race_labels) {
  file_path <- file.path("data", paste0("mort", year, ".csv"))
  data <- read.csv(file_path, fileEncoding="latin1")
...
f3 <- function(year, race_levels, race_labels) {
  file_path <- file.path("data", paste0("mort", year, ".csv"))
  data <- read.csv(file_path, fileEncoding="latin1")
  overseas_codes <- c("PR", "VI", "GU", "AS", "MP")
  summary_data <- data %>%
    ...
    select(monthdth, sex, age_group, race, stateoc_fips, countyoc_fips, ucod) %>%
    rename(cause = ucod, age = age_group, month = monthdth, fips = countyoc_fips, state = stateoc_fips) %>%
    mutate(state = ifelse(state %in% overseas_codes, "Overseas areas", state)) %>%
    mutate(sex = ifelse(sex == "M", "Male", "Female")) %>%
    group_by(month, state, fips, age, sex, race, cause) %>%
    summarise(deaths = n(), .groups = 'drop') %>%
...
f4 <- function(year, race_levels, race_labels) {
  file_path <- file.path("data", paste0("mort", year, ".csv"))
  data <- read.csv(file_path, fileEncoding="latin1")
  overseas_codes <- c("PR", "VI", "GU", "AS", "MP")
  summary_data <- data %>%
    ...
    select(monthdth, sex, age_group, racer40, stateoc_fips, countyoc_fips, ucod) %>%
    rename(cause = ucod, age = age_group, race = racer40, month = monthdth, fips = countyoc_fips, state = stateoc_fips) %>%
    mutate(state = ifelse(state %in% overseas_codes, "Overseas areas", state)) %>%
    mutate(sex = ifelse(sex == "M", "Male", "Female")) %>%
    group_by(month, state, fips, age, sex, race, cause) %>%
    summarise(deaths = n(), .groups = 'drop') %>%
...
f5 <- function(year, race_levels, race_labels) {
  file_path <- file.path("data", paste0("mort", year, ".csv"))
  data <- read.csv(file_path, fileEncoding="latin1")
  overseas_codes <- c("PR", "VI", "GU", "AS", "MP")
  summary_data <- data %>%
    ...
    select(monthdth, sex, age_group, racer5, stateoc_fips, countyoc_fips, ucod) %>%
    rename(cause = ucod, age = age_group, race = racer5, month = monthdth, fips = countyoc_fips, state = stateoc_fips) %>%
    mutate(state = ifelse(state %in% overseas_codes, "Overseas areas", state)) %>%
    mutate(sex = ifelse(sex == "M", "Male", "Female")) %>%
    group_by(month, state, fips, age, sex, race, cause) %>%
    summarise(deaths = n(), .groups = 'drop') %>%
...
summarize_files <- function(file_list, start_year, end_year) {
  selected_files <- file_list[grepl(paste0(start_year:end_year, collapse = "|"), file_list)]
  combined_data <- lapply(selected_files, function(file) {
    data <- tryCatch({
      readRDS(file)
    }, error = function(e) {
      NULL
